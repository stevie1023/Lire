...
def main(rank, args):
    ...
    if "alpaca" in base_model:
        model = LlamaForCausalLM.from_pretrained(
            base_model,
            torch_dtype=torch.bfloat16,
        )
        adapter_folder = (
            "/private/home/zhumingye/code/RRHF/results/rso_4res_e2_i2/checkpoint-588"
        )
        ...
        lora_weight = model.base_model.model.model.layers[0].self_attn.q_proj.weight
        ...
        for idx in range(len(inputs_string)):
            ...
            for i in range(8):
                temp.append(
                    [
                        inputs_string[idx],
                        outputs_string[1 * idx + i].replace(inputs_string[idx], ""),
                    ]
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--base_model",
        default="/data/rmw/RRHF/pretrained-models/alpaca-7b",
        type=str,
        help="model path",
    )
    parser.add_argument(
        "--data_path",
        default="/data/zhumingye/RRHF/data/rm-static/data",
        type=str,
        help="config path",
    )
    parser.add_argument("--batch_size", type=int, default=16, help="batch size")
    parser.add_argument("--port", type=int, default=22, help="batch size")
    # parser.add_argument("--diverse_beam", type=int, default=1, help="batch size")
    parser.add_argument(
        "--out_path",
        default="/data/rmw/RRHF/data/augmented",
        type=str,
        help="config path",
    )
